...
// ═══════════════════════════════════════════════════════════════════
// SANDRA IA MOBILE - SERVICE WORKER BUILDER
// Production Service Worker Generator for sandra.guestsvalencia.es
// ═══════════════════════════════════════════════════════════════════
...
const version = process.env.SW_VERSION || '98.0.0';
  ...
  switch (env) {
    case 'production':
      return {
        ...
        apiBase: 'https://sandra.guestsvalencia.es/api'
      };
    case 'staging':
      return {
        domain: 'deploy-preview--sandra-mobile.netlify.app',
        url: 'https://deploy-preview--sandra-mobile.netlify.app',
        apiBase: 'https://deploy-preview--sandra-mobile.netlify.app/api'
      };
    default:
      return {
...
// Generate production-ready service worker
const generateServiceWorker = () => {
  const domainConfig = getDomainConfig();
  const swVersion = `sandra-mobile-galaxy-${version}-${env}`;

  return `// ═══════════════════════════════════════════════════════════════════
// SANDRA IA MOBILE GALAXY - SERVICE WORKER
// Galaxy Level PWA Service Worker ${version}
// Environment: ${env}
// Domain: ${domainConfig.domain}
// Generated: ${new Date().toISOString()}
// ═══════════════════════════════════════════════════════════════════

const CACHE_NAME = '${swVersion}';
const STATIC_CACHE = 'sandra-static-${version}';
const DYNAMIC_CACHE = 'sandra-dynamic-${version}';
const API_CACHE = 'sandra-api-${version}';
const RUNTIME_CACHE = 'sandra-runtime-${version}';

// Environment configuration
const ENV = '${env}';
const DOMAIN = '${domainConfig.domain}';
const BASE_URL = '${domainConfig.url}';
const API_BASE = '${domainConfig.apiBase}';
...
async function cleanOldCaches() {
    ...
    const oldCaches = cacheNames.filter(name =>
        name.startsWith('sandra-') && !name.includes('${version}')
    );
...
self.addEventListener('notificationclick', event => {
    ...
    if (event.action === 'open') {
        event.waitUntil(
            clients.openWindow('${domainConfig.url}/sandra-ia-mobile-galaxy-responsive.html')
        );
    }
});
...
const main = () => {
  try {
    console.log(`🔧 Building Service Worker for ${env} environment...`);
    ...
    console.log(`✅ Service Worker built successfully: ${swPath}`);
...
module.exports = { generateServiceWorker, getDomainConfig };