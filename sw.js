...
// Generated: 2025-10-25T02:07:32.630Z
// ═══════════════════════════════════════════════════════════════════
...
const API_BASE = 'https://sandra.guestsvalencia.es/api';
...
self.addEventListener('install', event => {
    ...
    event.waitUntil(
        Promise.all([
            // Cache static files with iOS-specific handling
            caches.open(STATIC_CACHE).then(cache => {
                console.log('💾 Caching static files...');
                const filesToCache = STATIC_FILES.filter(url => {
                    try {
                ...
                // For iOS Safari, cache files one by one to avoid batch failures
                if (IS_IOS_SAFARI) {
