...
try {
  ...
  console.warn('[MCP] AI Gateway experimental no disponible:', e.message);
  AIGateway = null;
}
...
class SandraMCPBridge {
    ...
    initializeServer() {
        ...
        // ============================================================================
        // AI GATEWAY TOOLS (OpenAI / Anthropic - EXPERIMENTAL)
        // ============================================================================
        this.app.get('/api/ai/models', (_req, res) => {
            try {
                if (!AIGateway) {
                    return res.status(503).json({ success: false, error: 'AI Gateway no disponible' });
                }
        ...
        this.app.post('/api/ai/chat', async (req, res) => {
            try {
                ...
                if (!provider || !model || !messages) {
                    return res.status(400).json({ success: false, error: 'provider, model y messages son requeridos' });
                }
                const result = await AIGateway.chat({ provider, model, messages });
                res.json({ success: true, ...result });
            } catch (e) { 
                console.error('[MCP] AI Chat error:', e);
